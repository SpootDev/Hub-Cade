...
##                        fileHash = "0"+fileHash
    ...
    ##                    hash_dict[zippedFile] = fileHash,sha1_hash_data
                        #self.hash_result = self.file_name,fileHash,sha1_hash_data
                        print "zip: ",self.file_name,sha1_hash_data
                    except:
                        lock.acquire()
...
##                        lock.release()
                zip.close()
                #print "lenL:",len(hash_dict),hash_dict
                if len(hash_dict) > 0:
                    if len(hash_dict) == 1:
...
##                        fileHash = "0"+fileHash
    ...
    ##                    hash_dict[filename] = fileHash,sha1_hash_data
                        print "7z: ",self.file_name,sha1_hash_data
                        #self.hash_result = self.file_name,fileHash,sha1_hash_data
                    except:
                        lock.acquire()
                        Client_GlobalData.skipped_files.append(os.path.normpath(self.file_name) + "|Error on SHA1 of file")
...
##                while(len(fileHash)<8):
##                    fileHash = "0"+fileHash
##                hash_dict[os.path.basename(self.file_name)] = fileHash,sha1_hash_data
                if len(hash_dict) > 0:
                    fileHASHListSingle.append(hash_dict.values()[0])
                    fileHASHNameListSingle.append(os.path.normpath(self.file_name))
                file_pointer.close()
...
# from hash calc           hashFileMap[fileHash] = files[ (fileHash,self.file_name) ] = self.file_name
        ...
        print "Begin sql load Time: ",time.strftime("%Y:%m:%d_%H:%M:%S")
        ##curs_game.execute("select gir_gi_id,gir_rom_name,gir_crc32,gir_sha1 from game_info,game_info_roms where gi_id = gir_gi_id and gi_id IN (select gir_gi_id from game_info_roms group by gir_gi_id having count(*) > 1)")
        ...
        for sql_row in curs_game:
            #print "sql:",sql_row[0],sql_row[1],sql_row[2],sql_row[3]
            if first_rec == True:
            ...
            if old_gir_gi_id != sql_row[0]:
                ...
                temp_dict = {}
                old_gir_gi_id = sql_row[0]
            ##temp_dict[sql_row[1]] = sql_row[2],sql_row[3]
##            if sql_row[0] == 29:
##                print "rom: ",sql_row[1],sql_row[2],sql_row[3]
            # don't put the rom file in the list if it's a merged one
            if len(sql_row[3]) < 2:
##                if sql_row[0] == 29:
##                    print "rom2: ",sql_row[1],sql_row[2],sql_row[3]
                temp_dict[sql_row[1]] = sql_row[2]
        # catch last data from db
        if old_gir_gi_id == None:
            temp_dat = old_gir_gi_id,temp_dict
            #print "wtf:",temp_dat
            db_full_hash_dict.append(temp_dat)

        print "End sql load Time: ",time.strftime("%Y:%m:%d_%H:%M:%S")
        ...
        print "multi count:",len(fileHASHList)
        item_ndx = 0
...
##            print "XXX-",rom_hash_data
            rom_hash_length = len(rom_hash_data)
            for db_hash_dict in db_full_hash_dict:
##                if (db_hash_dict[0] == 409957 or db_hash_dict[0] == 562163 or db_hash_dict[0] == 562165 or db_hash_dict[0] == 562166):
##                    print "wtf:",db_hash_dict[0]
##                    print "ZZZ-",db_hash_dict[1]
                #print "ZZZ-",db_hash_dict[0]
##                if (self.rom_diff(rom_hash_data,db_hash_dict[1]) == True):
##                    print "I match bitch",rom_hash_data,db_hash_dict[1],db_hash_dict[0]
                #print "zzzz",db_hash_dict
...
##                    print "comp:",rom_hash_data,db_hash_dict[1],db_hash_dict[0]
                    ...
                    break
            item_ndx += 1
        print "Begin sql 2 load Time: ",time.strftime("%Y:%m:%d_%H:%M:%S")
        # do parse for single rom archives and files
        db_full_hash_dict = []
        temp_list = []
        ##curs_game.execute("select gir_gi_id,gir_crc32,gir_sha1 from game_info,game_info_roms where gi_id = gir_gi_id and gi_id IN (select gir_gi_id from game_info_roms group by gir_gi_id having count(*) = 1)")
        ...
        for sql_row in curs_game:
            ...
            temp_list = []
        print "End sql 2 load Time: ",time.strftime("%Y:%m:%d_%H:%M:%S")
        print "multi count2:",len(fileHASHListSingle)
        item_ndx = 0
        for rom_hash_data in fileHASHListSingle:
            Client_GlobalData.matching_on_file += 1
            for db_hash_dict in db_full_hash_dict:
                # check sha1 value
                ##if (rom_hash_data == db_hash_dict[2] or rom_hash_data == db_hash_dict[1]):
                if (rom_hash_data == db_hash_dict[1]):
                    ...
                    break
            item_ndx += 1
        print "about to store:",len(Client_GlobalData.found_rom_ids)
        curs_player.close()
        conn_player.close()
        curs_game.close()
        conn_game.close()
...
##        self.cartFileMaps = {}
    ...
    def load_hash_map_from_database(self):
        ...
        for sql_row in curs_game:
            ...
            #print "sql:",sql_row[0],sql_row[1],sql_row[2],sql_row[3],sql_row[4],sql_row[5],sql_row[6]
            game_times_played = 0
            game_time_played = 0
            game_monitor = "NA"
            game_players = 0
        ...
        # catch last data from db
        if old_system_long_name != None and len(game_info) > 0:
            Client_GlobalData.audit_gameList[old_system_long_name] = copy.deepcopy(game_info.items())
            Client_GlobalData.audit_gameList[old_system_long_name].sort()
    ...
    def getNamesDictDB(self,subString):
        ...
        for gameSystem in Client_GlobalData.audit_gameList.iteritems():
            ...
            #print "system:",gameSystem[0]
            for gameData in gameSystem[1]:
                #print "all:",gameData
                #print "game name:",gameData[0]
                #print "lower:",subString,gameData[0].lower()
                #print "stats:",gameData[0],"A",gameData[1][0],"B",gameData[1][1],"C",gameData[1][2],"D",gameData[1][3],"E",gameData[1][4],"F",gameData[1][5]
                if (Client_GlobalData.app.mainFrame.monitor_type_combo.GetValue() == "Horizontal" and gameData[1][2] != "Horizontal") or (Client_GlobalData.app.mainFrame.monitor_type_combo.GetValue() == "Vertical" and gameData[1][2] != "Vertical") or (int(gameData[1][3]) < Client_GlobalData.app.mainFrame.filter_player_count_spinner.GetValue()) or (Client_GlobalData.app.mainFrame.filterjoincategorychoice.GetStringSelection() != "All" and Client_GlobalData.app.mainFrame.filterjoincategorychoice.GetStringSelection() != gameData[1][5]):
                    ...
                    if subString in gameData[0].lower():
                        ...
                        #print "stats:",gameData[0],gameData[1][0],gameData[1][1],gameData[1][2],gameData[1][3],gameData[1][4],gameData[1][5]
