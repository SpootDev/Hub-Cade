...
class WinLayout(GladeSupport, WahCade):
    ...
    def __init__(self, glade_filename, window_name, config_opts, config_args):
        """build the window"""
        self.title = 'Hub!Cade Layout Editor'
        ...
        if self.config_opts.use_app_config:
            ...
            if not os.path.exists(self.config_dir):
                sys.exit(('No Hub!Cade config files found.  Run wahcade or wahcade-setup first.'))
        ...
        self.winMain.show()
        #properties dialogs
        self.dlg_props = DlgProps(wc_constants.LAYOUT_GLADE_FILE, 'dlgProps', self)
        self.dlg_props.populate_names(self._main_widgets)
        self.dlg_props.dlgProps.hide()
        self.dlg_screen = DlgScreenProps(wc_constants.LAYOUT_GLADE_FILE, 'dlgScreenProps', self)
        ...
        self._layout_windows = [
            ...
            (-1, self.fixdScr)]
        self._layout_items = [
            # the first digit is the line number in the layout file
            ...
            # begin hub!cade options
            (552, main_widgets['Video']),
            (565, main_widgets['Chat']),
            (578, main_widgets['Player(s)']),
            (591, main_widgets['Join']),
            # 604 will be the next item to add
            # opt widgets
    ...
    def on_mnuFSaveAs_activate(self, *args):
        """save hubcade layout file as"""
        self.save_layout_dialog(
            self.layout_filename,
            ('Save Hub!Cade Layout'),
            self.save_layout_file)
        self.set_window_title()
    ...
    def open_layout_dialog(self, default_filename, dlg_title, load_function):
        """open hubcade / cpviewer layout file dialog"""
    ...
    def save_layout_dialog(self, default_filename, dlg_title, save_function):
        """save hubcade / cpviewer layout file as... dialog"""
        dlg = gtk.FileChooserDialog(
            title = dlg_title,
    ...
    def load_layout_file(self, layout_filename):
        ...
        # if length is short means more options are available that isn't in the layout files so add new sections to "convert" them to hub!cade
        while len(lines) < 603:
            lines.append("False")
            lines.append(" 1")
            lines.append(" 0")
            lines.append(" 16777215")
            lines.append("SF Atarian System Extended")
            lines.append("False")
            lines.append("False")
            lines.append(" 12")
            lines.append(" 2")
