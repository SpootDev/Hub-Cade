...
def get_catver_item(rom_name):
    ...
    #get category info
    try:
        category = catver.get('Category', rom_name)
    except (ConfigParser.NoOptionError, ConfigParser.NoSectionError):
        category = 'Unknown'
    return category.decode('latin1')
...
def get_catver_ini(category_file):
    ...
    catver = ConfigParser.SafeConfigParser()
    catver.read(category_file)
    return catver
...
def create_initial_filter(info_filename, filter_filename, list_filename, emu_ini, pgb_pulse=None):
    ...
    if emu_ini.get('list_generation_method') == 'rom_folder':
        ...
        for rom_filename in roms:
            ...
            #doneemu_ini.get('list_generation_method')
            #write filter file
                ...
                if section in filter_spec and section in gd:
                    ...
                    if gd[section] in filter_spec[section]:
                        if not filter_spec[section][gd[section]]:
                            ...
                            matched = False
                    #if gd[section] is None or gd[section] == '':
                    #    if gd['rom_name'] == 'ASTEROID':
                    #        print "cfl: not matched (is none)"
                    #    matched = False
        #if gd['rom_name'] == 'ASTEROID':
        #    print "cfl: matched=",matched
        if matched:
            #write output
            add_game_to_filtered_list(gd, f)
    #done, close file
